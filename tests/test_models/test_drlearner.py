...
from uplift_forecast.models import DRLearner


def _model(**kwargs) -> DRLearner:
...
def test_global_propensity(uplift_data):
    x, treatment, y = uplift_data
    model = _model(propensity_model=None).fit(x, treatment, y)
...
def test_integration_with_uplift_forecast(uplift_data):
    x, treatment, y = uplift_data
    forecast = UpliftForecast(models=[_model(alias='dr')])
    forecast.fit(x, treatment, y)
    assert 'uplift_dr' in forecast.predict(x).columns
