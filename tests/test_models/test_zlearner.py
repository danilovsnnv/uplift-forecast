...
from uplift_forecast.models import ZLearner


def _model(**kwargs) -> ZLearner:
...
def test_integration_with_uplift_forecast(uplift_data):
    x, treatment, y = uplift_data
    forecast = UpliftForecast(models=[_model(alias='zl')])
    forecast.fit(x, treatment, y)
    assert 'uplift_zl' in forecast.predict(x).columns
